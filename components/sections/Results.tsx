    ...
    <section id="gallery" className="px-4 py-20 md:px-6">
      <div className="mx-auto max-w-6xl space-y-10">
        {copy.results.items.map((cs, i) => (
          <FadeIn key={i}>
            <div className={`soft-ui-card overflow-hidden rounded-[2.5rem] p-5 md:grid md:grid-cols-2 md:items-center md:gap-10 md:p-6 ${i % 2 === 1 ? 'md:[&>*:first-child]:order-2' : ''}`}>
              <div className="p-2 md:p-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  {cs.metrics.map(m => (
                    <div key={m.label} className="rounded-[1.8rem] border border-white/55 bg-white/55 px-4 py-4 backdrop-blur">
              ...
              <div className="editorial-shadow relative mt-6 aspect-[4/3] overflow-hidden rounded-[2.15rem] md:mt-0">
                <Image
                  src={photos[i]}
                  alt={cs.photoAlt}
                  fill
